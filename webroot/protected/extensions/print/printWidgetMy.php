 ...
 */
 ...
 class printWidgetMy extends CWidget
{
    /**
    ...
    */
    ...
    public function init()
    {
        ...
        echo '<div style="float:'. $this->align .';" align="' . $this->align . '">';
        echo CHtml::link("<img src=\"$imgUrl\" class=\"img_link\" title=\"".Yii::t('lms', 'Print this page')."\">",
                        "#", array_merge($this->htmlOptions, array('onclick'=>"return processPrint('$this->printedElement');")));
        echo '</div>';
        self::$count++;
        if (self::$count == 1) {
            ...
            $script_text="
                ...
                function processPrint(param)
                {
                    ...
                    {
                        jQuery('.printWidgetPrintedElement').remove();
                    }
                    arrayEl = jQuery(param).get();
                    for (var i=0; i<arrayEl.length; i++)
                    {
                        clonedEl = jQuery(arrayEl[i]).clone();
            ...
            $css_print=" .printWidgetScreenCover {display: none;}\n .printWidgetPrintedElement {display: block; margin: 10px;}";
            $cs->registerScript('Print Bottom',$script_text,CClientScript::POS_END);
            $cs->registerCss('print',$css_print,'print');
            $css_screen=" .printWidgetPrintedElement {display: none;}\n .no_screen {display: none;}";
            $cs->registerCss('screen',$css_screen,'screen');
        }
    }
